...
fn main() -> io::Result<()> {
    ...
    let project_root = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../..").canonicalize()?;
    assert!(project_root.join(".git/HEAD").exists(), "devtool has been moved?");
    ...
    let out_path = {
        ...
        PathBuf::from(out_dir).join("buildinfo.rs")
    };
