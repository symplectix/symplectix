...
#[test]
fn sum_empty() {
    assert_eq!(0, xf::sum.fold(0, empty::<i32>()));
    assert_eq!(1, xf::sum.fold(1, empty::<i32>()));
}

#[test]
fn sum_once() {
    assert_eq!(1, xf::sum.fold(0, once::<i32>(1)));
    assert_eq!(2, xf::sum.fold(0, once::<i32>(2)));
}
...
fn par_conj_sum() {
    ...
    let g = xf::map(mul3).take(2).apply(xf::sum);
